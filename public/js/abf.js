...
class abfIntegration{
	async query(method, route, data = null){
		let params = {
		  type: method,
		  url: route,
		  dataType: "json"
		...
		try{
			let response = await $.ajax(params);
	...
	//32.905.377/0001-86
	async authPage(){
		function validateData({value}, {data}){
			...
			if(!search){
				...
				if(search.SituationABF !== "Ativa" && search.Status !== "Associado"){
					throwError("Sua situação de associado se encontra é: "+search.SituationABF);
					$(".btnAssociates button:first-child").hide();
					$(".btnAssociates button:last-child").show();
					toggleOptions(false);
				}else{
					...
					$('select').append(`<option value="${search.TradeName}">${search.TradeName}</option>`)
		...
		function throwError(msg = null){
			if(!document.querySelector('.has-error')){
				$(".shippingAuthentication input[type=text]").after(`<span class="has-error"></span>`)
			}else{
				if(msg){
		...
		function toggleOptions(def){
			if(def){
				$(".shippingAuthentication select#brandAuthentication").show();
				$(".wrapFormAuthentication .nice-select").show();
			}else{
				$(".shippingAuthentication select#brandAuthentication").hide();
				$(".wrapFormAuthentication .nice-select").hide();	
			}
		}
		...
		$(".shippingAuthentication input[type=text]").change(({target: value}) => validateData(value, database))
...
window.addEventListener("load", () => setTimeout(() => new abfIntegration().init(), 500))
